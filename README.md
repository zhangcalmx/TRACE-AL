...
- **评分层级说明**：`configs/rules.yaml` 的 `aggregation_semantics` 描述的是基础触发引擎自身的内部聚合（严重度加权、≥4），该层已被冻结 v1.4 策略覆盖——演示与论文的累计评分（满分 40）与主预警（≥12）均由策略层执行。`rules.yaml` 为哈希锚定文件（sha256 与验收记录一致），保持字节不变
